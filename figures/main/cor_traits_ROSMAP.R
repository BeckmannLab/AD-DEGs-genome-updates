...
# ==== Select Subtraits and Define Formula ====
subtraits <- infoall2[, c("braaksc", "ceradsc", "cogdx")]
form <- ~ braaksc + ceradsc + cogdx
...
canCorPairsTest <- function (formula, data, showWarnings = TRUE, number_perm = 1000) {
    ...
    if (variancePartition:::.isMixedModelFormula(formula)) {
        stop("Invalid formula: mixed effects not supported.")
    }
...
# ==== Correlation Matrix and P-value Adjustment ====
...
rownames(ccFit$C) <- colnames(ccFit$C) <- c("Braak", "CERAD", "Cognitive")

# ==== Save Correlation Matrix Plot ====
pdf(file.path(plot_dir, "cor_traits_rosmap.pdf"))
plotCorrMatrix(ccFit$C, cellnote = round(ccFit$C, 2), notecex = 1.5, notecol = "black")
dev.off()
