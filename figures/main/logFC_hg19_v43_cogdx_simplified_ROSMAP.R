...
all_results$v43hg19DE_cogdx_simplified = gsub("v43Signif", "GRCh38.p13 significant",all_results$v43hg19DE_cogdx_simplified )

all_results$v43hg19DE_cogdx_simplified = gsub("hg19Signif", "GRCh37 significant",all_results$v43hg19DE_cogdx_simplified )

all_results$v43hg19DE_cogdx_simplified = gsub("bothSignif", "Both references significant",all_results$v43hg19DE_cogdx_simplified )

all_results$v43hg19DE_cogdx_simplified = gsub("notSignif", "Neither reference significant",all_results$v43hg19DE_cogdx_simplified )



g <- ggplot(all_results, aes(`logFC|cogdx_simplified_hg19`, `logFC|cogdx_simplified_v43`)) +
  # layer 1: neither (grey) + both (green)
  geom_point(
    data = subset(all_results,
      v43hg19DE_cogdx_simplified %in% c("Neither reference significant","Both references significant")
    ),
    aes(color = v43hg19DE_cogdx_simplified),
    size  = 2.5,
    alpha = 0.4
  ) +
  ...
  geom_point(
    data = subset(all_results,
      v43hg19DE_cogdx_simplified %in% c("GRCh37 significant","GRCh38.p13 significant")
    ),
    aes(color = v43hg19DE_cogdx_simplified),
    size  = 2.5,
    alpha = 0.4
  ...
  scale_color_manual(
    ...
    values = c(
      ...
      "GRCh37 significant"             = "red",
      "GRCh38.p13 significant"         = "blue"
    ),
    guide = guide_legend(
      override.aes   = list(size = 6),   # slightly smaller symbols
      title.position = "top",
      title.hjust    = 0
    )
  ) +
  ...
  theme(
    ...
    # legend styling: a bit smaller than “supersized”
    legend.position      = c(0.36, 0.83),
    legend.direction     = "vertical",
    legend.title         = element_text(size = 20),     # slightly reduced
    legend.text          = element_text(size = 16),     # slightly reduced
    legend.key.size      = unit(1.5, "lines"),          # back down a notch
    legend.background    = element_rect(color = "black", fill = "white")
    # legend.title.align   = 0,
    # panel.spacing      = unit(0.5, "lines"),
    # plot.margin        = margin(10, 10, 10, 10)
  ) + coord_fixed(    xlim   = c(-0.6,  0.6),
    ylim   = c(-0.6,  0.8))

ggsave("logFC_hg19_v43_cogdx_simplified.pdf",g, width = 7, height = 7, useDingbats=FALSE)
