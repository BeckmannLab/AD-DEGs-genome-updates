...
msbb_all_results=as.data.frame(all_results)

rownames(msbb_all_results) <- msbb_all_results$V1
msbb_all_results$V1 <- NULL

# 2. Pull out just the logFC columns once
msbb_logfc_subset <- msbb_all_results[, grep("logFC", colnames(msbb_all_results)), drop = FALSE]
...
corr_list <- list()
for (trait in traits) {
  raw_cols <- grep(trait, colnames(msbb_logfc_subset), value = TRUE, fixed = TRUE)
  if (length(raw_cols) < 2) next
  mat <- msbb_logfc_subset[, raw_cols, drop = FALSE]
...
out_pdf <- "msbb_cor_DE_AD_all_traits.pdf"
