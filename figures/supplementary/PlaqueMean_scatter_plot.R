...
plot_data <- do.call(rbind, Map(function(comp, old, new) {
  xcol  <- paste0("logFC|PlaqueMean_test.txt_", old)
  ycol  <- paste0("logFC|PlaqueMean_test.txt_", new)
  decol <- paste0(new, old, "DE_PlaqueMean_test.txt")
...
ggsave("PlaqueMean_scatter_plot.pdf",g, useDingbats = FALSE, width = 17.5, height= 9)
