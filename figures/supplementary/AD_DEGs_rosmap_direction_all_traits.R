...
combo_var <- read.delim(
  "/sc/arion/projects/mscic1/results/anina/fun_project_4.23/combo_var.txt",
...
var <- read.delim(
  "/sc/arion/projects/mscic1/results/anina/fun_project_4.23/analysis/var.txt",
  header = FALSE, sep = "\t", stringsAsFactors = FALSE
)
...
# structure(list(V1 = c("hg19hg18DE_ceradsc_defvsctl", "v30hg18DE_ceradsc_defvsctl", 
# "v43hg18DE_ceradsc_defvsctl", "v30hg19DE_ceradsc_defvsctl", "v43hg19DE_ceradsc_defvsctl", 
# "v43v30DE_ceradsc_defvsctl", "T2Thg18DE_ceradsc_defvsctl", "T2Thg19DE_ceradsc_defvsctl", 
...
# "T2TSignif", "T2TSignif", "T2TSignif", "T2TSignif")), row.names = c(NA, 
...
overview_t$assembly <- factor(overview_t$assembly, levels = combo_var[,1])
overview_t$trait    <- factor(overview_t$trait,    levels = traits)

# ==== FIX HERE: rename & factor the comparison column, not “name” ====
...
ggsave(
  filename    = "/hpc/users/lunda02/www/plots/assembly_paper/AD/AD_DEGs_rosmap_direction_all_traits.pdf",
  plot        = g,
  useDingbats = FALSE,
  width       = 16,
  height      = 8
