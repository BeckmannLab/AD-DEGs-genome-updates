...
# 2) Define comparisons and reference IDs
comparisons <- c(
...
plot_data <- do.call(rbind, Map(function(comp, old, new) {
  xcol  <- paste0("logFC|braaksc_simplified_", old)
  ycol  <- paste0("logFC|braaksc_simplified_", new)
  decol <- paste0(new, old, "DE_braaksc_simplified")
...
ggsave("braaksc_simplified_scatter_plot.pdf",g, useDingbats = FALSE, width = 17.5, height= 9)
