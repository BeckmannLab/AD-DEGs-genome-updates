...
traits = c("braaksc_simplified", "cogdx_simplified", "ceradsc_simplified")
...
for (i in 1:nrow(all)){
	...
	write.table(back, paste0("rosmap_common_",all[i,1],"_", all[i,2], "_background_5.13.24.txt"), row.names = T, sep = ",")
	df = as.data.frame(for_gsea_df$gene_ID[for_gsea_df$DE == "Signif"])
	write.table(df, paste0("rosmap_common_only_sig","_",all[i,1],"_",all[i,2], "_5.13.24.txt"), row.names = T, sep = ",")
}

##set up
gene_list_files=list.files("/input",pattern="rosmap_common_only")
universe_files=list.files("/input",pattern="background")
all2 = cbind(gene_list_files, universe_files)
all2 = as.data.frame(all2)
all2$file_name = gsub("rosmap_common_only_sig_", "",all2$gene_list_files)
all2$file_name = gsub("_5.13.24.txt", "",all2$file_name)

##run enrichment
for (i in 1:nrow(all2)){
	var1=paste0("/input/",all2[i,1])
	var2=paste0("/input/",all2[i,2])
...
saveRDS(all,"rosmap_final_chromsome.RDS")
