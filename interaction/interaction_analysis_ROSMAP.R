...
##load raw counts
	...
	all_var <- read.delim("/sc/arion/projects/mscic1/results/anina/fun_project_4.23/msbb/all_var.txt", sep = "\t", header = F)
...
#     )), class = "data.frame", row.names = c(NA, -10L))
	...
	info_all_base=readRDS("infoall_09.13.23.RDS") #meta
...
res_final=foreach(i = seq_len(nrow(all_var))) %dopar% {
	if ((all_var[i,3] %in% c("genedata_hg18", "genedata_hg19","genedata_v30", "genedata_v43"))){
		...
		#run dream
			...
			##formula
			formVoom = formula(paste0("~ assembly + (1|Batch) + RINcontinuous + RnaSeqMetrics__MEDIAN_5PRIME_TO_3PRIME_BIAS + (1|msex) + AlignmentSummaryMetrics__STRAND_BALANCE + RnaSeqMetrics__PCT_INTRONIC_BASES + pmi + (1|race)"))
			...
			disease_statuses=c("cts_mmse30_lv","braaksc_simplified","cogdx_simplified","ceradsc_defvsctl")
			...
			if(status!="ceradsc_defvsctl"){
		        ...
		        resVPtmp=nonvoom0.5[,which(is.na(info_all2[,"ceradsc"])==F)]
		    }
		    ...
		    ##you need a interaction term with a continous variable (mmse30) becasue otherwise it doesnt make sense. we do 0 here so the direction is clear but not necessary. 
		    if(status=="cts_mmse30_lv"){
		        formula_test=formula(paste("~ ",status," * assembly + (1|Batch) + RINcontinuous + RnaSeqMetrics__MEDIAN_5PRIME_TO_3PRIME_BIAS + (1|msex) + AlignmentSummaryMetrics__STRAND_BALANCE + RnaSeqMetrics__PCT_INTRONIC_BASES + pmi + (1|race)",sep=""))
		    }else if(status!="ceradsc_defvsctl"){
		        formula_test=formula(paste("~ 0 + ",status," * assembly + (1|Batch) + RINcontinuous + RnaSeqMetrics__MEDIAN_5PRIME_TO_3PRIME_BIAS + (1|msex) + AlignmentSummaryMetrics__STRAND_BALANCE + RnaSeqMetrics__PCT_INTRONIC_BASES + pmi + (1|race)",sep=""))
		    }else{
		        formula_test=formula(paste("~ 0 + ceradsc * assembly + (1|Batch) + RINcontinuous + RnaSeqMetrics__MEDIAN_5PRIME_TO_3PRIME_BIAS + (1|msex) + AlignmentSummaryMetrics__STRAND_BALANCE + RnaSeqMetrics__PCT_INTRONIC_BASES + pmi + (1|race)",sep=""))
		    }

			# important
			    design=model.matrix(formula(paste0("~",gsub("1 |","",as.character(formula_test)[2],fixed=TRUE))),info_alltmp) #this is important - where the values we care about are. don't care about contrast for interaction
		        ...
		        coef_regex="cts_mmse30_lv:assembly|ceradsc4:assembly|AD:assembly"
