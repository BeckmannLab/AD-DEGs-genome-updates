...
df_all2 <- df_all2[grepl("ceradsc_defvsctl|braaksc_simplified|cogdx_simplified|cts_mmse30", df_all2$trait), ]
df_all2$DEAD_status = ifelse(df_all2$DEAD_status != "notSignif", "Signif", "notSignif")
colnames(df_all2) = c("ID", "DE", "trait", "reference")

#create universe and sig files
for (x in unique(df_all2$trait)){
	for (y in unique(df_all2$reference)){
	print(x)
	df_all_input = df_all2[df_all2$trait == x & df_all2$reference == y ,]
	...
	write.table(subset_df2,paste0(x,"_",y,"_msigdb_singlecell_universe.txt"), row.names = FALSE)
	sig_subset_df = for_gsea_df[which(for_gsea_df$DE == "Signif"),]
	sig_subset_df2 =sig_subset_df$gene_ID
	write.table(sig_subset_df2,paste0(x,"_",y,"_msigdb_singlecell_sig.txt"),row.names = FALSE)
}}
...
for (x in unique(df_all2$trait)){
	for (y in unique(df_all2$reference))
	print(x)
	var1=paste0(x,"_",y,"_msigdb_singlecell_sig.txt")
	var2=paste0(x,"_",y,"_msigdb_singlecell_universe.txt")
	...
	var4="/c2"
	var5=paste0(x,"_",y, "_single_cell_msigdb.txt")
...
data_list <- lapply(files, function(f) {
  ...
  reference_pair <- sub(
  "^[^_]+_[^_]+_([^_]+_[^_]+).*","\\1", basename(f))
  
  # add as a new column
  df$trait <- trait
  df$reference_pair <- reference_pair
...
saveRDS(all, "c2_hallmark_msigdb_rosmap.RDS") #Supplementary Table 2: ROSMAP msig for each ref pair
