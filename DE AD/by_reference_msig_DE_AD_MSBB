...
data_list <- lapply(files, function(f) {
  ...
  df <- fread(f)
 	b_name = sub("PlaqueMean", "Plaque_Mean", basename(f))
  trait <- sub("^([^_]+_[^_]+).*", "\\1", b_name)
  reference_pair <- sub("^[^_]+_[^_]+_([^_]+_[^_]+).*","\\1", b_name)
...
saveRDS(all, "c2_hallmark_msigdb_msbb.RDS") #Supplementary Table 2: MSBB msigdb for each ref pair
